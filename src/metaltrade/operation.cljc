(ns metaltrade.operation
  "OperationActor -- one wholesale-metal operation = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (MetalTradeAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Metal Trading Governor
  ...
  One graph run = one wholesale-metal operation (intake -> advise ->
  govern -> decide -> commit | hold | approval). No unbounded inner
  loop -- each operation is auditable and checkpointed. A metal-order's
  ...
  node when the governor is clean -- see `metaltrade.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [metaltrade.metaltradeadvisor :as metaltradeadvisor]
            [metaltrade.governor :as governor]
...
(defn build
  ...
  `metaltrade.store/Store`).
  opts:
    :advisor      -- a `metaltrade.metaltradeadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (metaltradeadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; MetalTradeAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (metaltradeadvisor/-advise advisor store request)]
            {:proposal p :audit [(metaltradeadvisor/trace request p)]})))

      ;; Metal Trading Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
